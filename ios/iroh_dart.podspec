#
# iroh_dart iOS plugin. cargokit builds the owned `irohdart-ffi` staticlib (repo-root
...
Pod::Spec.new do |s|
  ...
  s.platform = :ios, '13.0'
  # With use_frameworks!, iroh_dart builds as a dynamic framework. cargokit puts the Rust staticlib
  # at $PODS_CONFIGURATION_BUILD_DIR/iroh_dart/libirohdart_ffi.a; -force_load links its whole archive
  # INTO the framework so the FFI symbols survive dead-stripping and are visible to
  # DynamicLibrary.process() at runtime.
  s.pod_target_xcconfig = {
    'DEFINES_MODULE' => 'YES',
    'OTHER_LDFLAGS' => '-force_load ${PODS_CONFIGURATION_BUILD_DIR}/iroh_dart/libirohdart_ffi.a',
  }
